	...
	<view class="register">
		<view class="content">
			...
			<view class="main">
				...
				<wInput v-model="userInfo.password" type="password" maxlength="11" placeholder="登录密码" isShowPass></wInput>
				<wInput v-model="userInfo.name" type="text" maxlength="11" placeholder="姓名"></wInput>
				<wInput v-model="userInfo.phone" type="number" maxlength="11" placeholder="手机号"></wInput>
				<!-- 	<wInput
					v-model="verCode"
					type="number"
					maxlength="4"
			...
			<wButton :text="ntb" :rotate="isRotate" @click.native="startReg()"></wButton>
...
export default {
	...
	onLoad(option) {
		this.ntb = option.nav == '添加用户' ? '添 加' : '注 册';
		uni.setNavigationBarTitle({
			title: option.nav
		});
		if (option.nav == '添加用户') {
			this.userInfo.pid = uni.getStorageSync('userInfo').id;
		}
	},
	mounted() {
		_this = this;
